...
async function suite(type,label,opts){
 ...
 await app.locator('#draft').fill(Array.from({length:1200},(_,i)=>'Sentence '+(i+1)+' exercises large-document analysis with representative text.').join(' '));const a0=Date.now();await app.getByRole('button',{name:'Analyze writing'}).click();ok(label+' large draft analysis under 5s',Date.now()-a0<5000,(Date.now()-a0)+'ms');await app.close();
 ...
 ok(label+' iPhone Lens fast actions',await bmPage.locator('#provenance-iphone-lens').getByRole('button',{name:'Locate question on Canvas'}).count()>0&&await bmPage.locator('#provenance-iphone-lens').getByRole('button',{name:'Copy Lens cue'}).count()>0&&await bmPage.locator('#provenance-iphone-lens').getByRole('button',{name:'Go to next Canvas question'}).count()>0);
