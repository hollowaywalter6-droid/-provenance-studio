...
check('Practice autofill is explicitly gated',overlay.includes('function practiceAutofillEnabled()')&&overlay.includes("data-provenance-autofill")&&overlay.includes("if(!practiceAutofillEnabled())return false"));
check('Practice autofill covers radio checkbox dropdown and text',overlay.includes('input[type="radio"],input[type="checkbox"]')&&overlay.includes("root.querySelectorAll('select')")&&overlay.includes('data-provenance-answer'));
check('Practice autofill controls are hidden outside practice mode',overlay.includes("if(practiceAutofillEnabled()){const fill=quickButton('Fill this'"));
check('One master mode control is present',overlay.includes('data-a="master"')&&index.includes('data-o aria-label="Toggle Provenance mode"'));
check('Practice master enables all three automation behaviors together',index.includes('one master ON/OFF control')&&overlay.includes("['select','fill','submit'].forEach"));
check('Live Canvas master does not enable assessment autofill',overlay.includes('let masterEnabled=!practiceAutofillEnabled()')&&overlay.includes('if(practiceAutofillEnabled()){'));
check('Practice auto-submit is sandbox-gated',overlay.includes("data-provenance-practice-submit")&&overlay.includes("if(!practiceAutofillEnabled())return false"));
