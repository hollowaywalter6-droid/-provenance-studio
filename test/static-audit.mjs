...
check('Canvas Lens omits current-selection UI',!overlay.includes('Currently selected in Canvas'));
check('Canvas Lens deduplicates nested questions',overlay.includes('questionSignature')&&overlay.includes('uniqueQuestions'));
check('Canvas Lens reads Canvas feedback',overlay.includes('choiceFeedback')&&overlay.includes('Canvas feedback identifies'));
check('Canvas Lens lists the full deduplicated question set',overlay.includes("all listed")&&overlay.includes("slice(0,100)"));
...
check('iPhone bookmarklet deduplicates and lists all questions',index.includes('function unique()')&&index.includes('__provenanceLensTimer')&&index.includes("all listed")&&index.includes("slice(0,100)"));
check('iPhone bookmarklet reads Canvas feedback',index.includes("Canvas feedback identifies:")&&index.includes("feedback(input,root)"));
...
check('Service worker cache is v4.1.0',sw.includes("provenance-v4-1-0"));
check('Version endpoint matches release',version.version==='4.1.0'&&index.includes("APP_VERSION='4.1.0'"));
