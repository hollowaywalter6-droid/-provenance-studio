...
async function appRun(name,viewport){
  ...
  const profileAfter={name:await page.locator('#profileName').inputValue(),email:await page.locator('#profileEmail').inputValue(),stored:await page.evaluate(()=>localStorage.getItem(PROFILE))};pass(name+' local profile survives reload',profileAfter.name==='Provenance QA'&&profileAfter.email==='qa@example.com',JSON.stringify(profileAfter));
