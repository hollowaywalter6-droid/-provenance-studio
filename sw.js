...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/version.json')){
    e.respondWith(fetch(new Request(e.request,{cache:'no-store'})).catch(()=>new Response(JSON.stringify({version:'4.5.0'}),{headers:{'content-type':'application/json'}})));
    return;
  }
