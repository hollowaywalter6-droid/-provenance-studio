...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/version.json')){
    e.respondWith(fetch(new Request(e.request,{cache:'no-store'})).catch(()=>new Response(JSON.stringify({version:'4.2.1'}),{headers:{'content-type':'application/json'}})));
    return;
  }
